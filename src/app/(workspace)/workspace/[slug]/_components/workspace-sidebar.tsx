...
import {
  Home, CircleUser, Settings, LogOut, Plus, ChevronsUpDown,
  Check, Tag, LayoutTemplate, Activity, Users, BarChart2, X,
} from "lucide-react";
import {
...
export function WorkspaceSidebar({
  ...
  const bottomLinks = [
    ...
    { href: `${baseSlug}/settings/labels`,   label: "Labels",    icon: Tag,            exact: false },
        ...
        <div className="flex items-center justify-between px-2.5 py-1">
          ...
          {canCreateProject ? (
            <CreateProjectDialog
              ...
              trigger={
                <Button variant="ghost" size="icon" className="size-5 text-muted-foreground hover:text-foreground" aria-label="Create project">
                  <Plus className="size-3.5" />
                </Button>
              }
            />
          ) : (
            <Button variant="ghost" size="icon" className="size-5 text-muted-foreground" disabled>
              <Plus className="size-3.5" />
            </Button>
          )}
        </div>
        {projects.length === 0 ? (
          <p className="px-2.5 py-1.5 text-xs text-muted-foreground">No projects yet</p>
        ) : (
          projects.map((project) => {
            const href = `${baseSlug}/projects/${project.key}`;
            return (
              <Link key={project.id} href={href} onClick={handleNavClick}
                className={cn(
                  "flex items-center gap-2.5 rounded-md px-2.5 py-1.5 text-sm font-medium transition-colors",
                  isActive(href) ? "bg-accent text-accent-foreground" : "text-sidebar-foreground/70 hover:bg-accent hover:text-accent-foreground",
                )}
              >
                <div className="flex size-5 shrink-0 items-center justify-center rounded bg-primary/10 text-[10px] font-bold text-primary">
        ...
        {user && (
          <div className="mt-1 flex items-center gap-2 rounded-md px-2.5 py-1.5">
            <Avatar className="size-7 shrink-0">
              ...
              <AvatarFallback className="text-xs font-semibold">{getInitials(user.name ?? user.email)}</AvatarFallback>
            </Avatar>

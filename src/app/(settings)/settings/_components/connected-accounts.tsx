...
export function ConnectedAccounts({ accounts: initialAccounts }: ConnectedAccountsProps) {
  ...
  function handleLink(providerId: string) {
    ...
    authClient.linkSocial({
      ...
      if (err) { setError(err.message ?? "Failed to link account."); setLoadingProvider(null); return; }
  ...
  function handleUnlink(providerId: string) {
    ...
    startTransition(async () => {
      ...
      if (err) { setError(err.message ?? "Failed to unlink account."); return; }
  ...
  return (
    <div className="flex flex-col gap-3">
      {PROVIDERS.map(({ id, label, icon: Icon }) => {
        ...
        const loading   = loadingProvider === id && isPending;
        ...
        return (
          ...
          >
            ...
            <Button
              ...
              onClick={() => connected ? handleUnlink(id) : handleLink(id)}
              className="gap-1.5"
            >
              {loading
